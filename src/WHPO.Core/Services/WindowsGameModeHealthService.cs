...
///   o el Presence Writer de Game Bar en System32 (GameBarPresenceWriter.dll en
///   builds ≤23H2; GameBarPresenceWriter.exe/.proxy.dll en 24H2+; la DLL real
///   vive dentro del paquete en las versiones nuevas) — el interruptor queda
///   muerto y NO hay regla de registro que lo reviva: hay que reinstalar Game
///   Bar (Store).
///
...
public sealed class WindowsGameModeHealthService : IWindowsGameModeHealthService
{
    ...
    // El Presence Writer de Game Bar cambió de layout entre builds de Windows:
    // hasta ~23H2 venía como System32\GameBarPresenceWriter.dll; en 24H2/25H2
    // Microsoft dejó GameBarPresenceWriter.exe + GameBarPresenceWriter.proxy.dll
    // en System32 y movió la DLL real adentro del paquete XboxGamingOverlay.
    // El Modo Juego está sano si cualquiera de los archivos de presencia existe.
    private static readonly string[] PresenceWriterCandidates =
    [
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "GameBarPresenceWriter.dll"),
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "GameBarPresenceWriter.exe"),
        Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "GameBarPresenceWriter.proxy.dll")
    ];

    private static bool HasPresenceWriterInfra() => PresenceWriterCandidates.Any(File.Exists);
    ...
    private static (bool? Master, List<(string Component, bool Present, string Detail)> Checks) ReadMasterSwitchAndChecks()
    {
        ...
        try
        {
            bool present = HasPresenceWriterInfra();
            checks.Add((
                "GameBarPresenceWriter (System32)",
                present,
                present ? "Presente (dll/exe del Presence Writer)" : "Ausente: Modo Juego huérfano"));
        }
        catch (Exception ex)
        {
