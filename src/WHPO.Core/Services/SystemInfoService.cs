...
public class SystemInfoService : ISystemInfoService, IDisposable
{
 ...
 // cae a la suma total de todos los adaptadores.
    ...
    public SecurityFeatures GetSecurityFeatures()
    {
        ...
        // --- VBS / HVCI: WMI Win32_DeviceGuard (root\Microsoft\Windows\DeviceGuard).
        // VirtualizationBasedSecurityStatus: 0 desactivado, 1 configurado sin correr,
        // 2 en ejecución. SecurityServicesConfigured / SecurityServicesRunning son
        // arrays de servicios de seguridad habilitados / en curso (nombres verificados
        // contra el reporte de msinfo32 de Windows): 1 = Credential Guard,
        // 2 = HVCI / integridad de memoria, 3 = Secure Launch, 4 = SMM Firmware
        // Measurement, 5 = protección de pila aplicada por hardware en modo kernel.
        // Solo interesa el 2 (HVCI) para la card de VBS.
        // OJO: el provider declara esas propiedades como UInt32Array, así que en .NET
        // llegan como UInt32[] y se normalizan explícitamente con SecurityServiceIds
        // (no depender del cast int[]↔uint[] del runtime, que es un detalle no
        // documentado: funciona, pero es frágil y reinterpreta los bits).
        // Si el namespace/clase no existe (Windows viejo, WMI dañado) o la clase no
        // tiene instancias, el estado es DESCONOCIDO (-1): no se puede afirmar que
        // esté desactivado (mismo criterio que el TPM, que reporta "No detectado").
        int vbsStatus = -1;
        bool hvciRunning = false, hvciConfigured = false;
        try
        {
            var scope = new ManagementScope(@"\\.\root\Microsoft\Windows\DeviceGuard");
            using var searcher = new ManagementObjectSearcher(scope,
                new ObjectQuery("SELECT VirtualizationBasedSecurityStatus, SecurityServicesConfigured, SecurityServicesRunning FROM Win32_DeviceGuard"));
            bool hasInstance = false;
            foreach (ManagementObject obj in searcher.Get())
            {
                hasInstance = true;
                if (obj["VirtualizationBasedSecurityStatus"] != null)
                    vbsStatus = Convert.ToInt32(obj["VirtualizationBasedSecurityStatus"]);
                hvciConfigured = SecurityServiceIds(obj["SecurityServicesConfigured"]).Contains(HvciServiceId);
                hvciRunning = SecurityServiceIds(obj["SecurityServicesRunning"]).Contains(HvciServiceId);
                break;
            }
            if (!hasInstance)
                _loggingService.LogWarning("VBS: Win32_DeviceGuard no devolvió instancias (estado desconocido).");
        }
        catch (Exception ex)
        {
            _loggingService.LogWarning($"VBS no detectado vía WMI: {ex.Message}");
        }

        return new SecurityFeatures(tpmPresent, tpmEnabled, tpmSpecVersion, uefiFirmware, secureBootEnabled, iommuPresent, vbsStatus, hvciRunning, hvciConfigured);
    }

    /// <summary>Id de HVCI (integridad de memoria) dentro de Win32_DeviceGuard.</summary>
    private const uint HvciServiceId = 2;

    /// <summary>
    /// Normaliza el array de ids que WMI expone en SecurityServicesConfigured /
    /// SecurityServicesRunning. El provider lo declara como UInt32Array (llega
    /// UInt32[] en .NET) — se aceptan también int[] y arrays genéricos por robustez
    /// ante cambios del provider. Devuelve vacío si el valor es null.
    /// </summary>
    private static uint[] SecurityServiceIds(object? value) => value switch
    {
        uint[] u => u,
        int[] i => Array.ConvertAll(i, v => (uint)v),
        Array a => a.Cast<object>().Select(v => v == null ? 0u : Convert.ToUInt32(v)).Where(v => v > 0).ToArray(),
        _ => Array.Empty<uint>()
    };
