...
public class TweakService : ITweakService
{
 ...
 // ====== Utilidades ======
    ...
    private async Task<TweakResult> RunCommandAsync(string command, string args = "")
    {
        try
        {
            ...
            // -OutputFormat Text: los errores de cmdlet en formato XML (clixml)
            // son ilegibles en la consola ("#< CLIXML ..."); en texto plano se
            // entiende qué falló.
            var effectiveArgs = command.Equals("powershell", StringComparison.OrdinalIgnoreCase)
                ? "-NoProfile -NonInteractive -OutputFormat Text " + args
                : args;
            ...
            {
                // El stderr suele venir vacío (scripts con exit explícito o
                // SilentlyContinue): caer a stdout para no devolver un
                // "Exit code 1: " pelado que no dice nada en la consola.
                var detail = error.Trim();
                if (string.IsNullOrEmpty(detail)) detail = output.Trim();
                if (string.IsNullOrEmpty(detail)) detail = "sin salida del comando";
                if (detail.Length > 600) detail = detail[..600] + " …";
                return new TweakResult(false, $"Exit code {process.ExitCode}: {detail}");
            }
 ...
 // comando, para que apply y check estén alineados. El aviso del paso va
 // ANTES de ejecutar (si el set cuelga, el último paso visto dice dónde).
                Report("STEP 1/5 ocultar IA en Configuracion");
                var reg1 = SetRegistryValue(RegistryHive.LocalMachine, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer", "SettingsPageVisibility", "hide:aicomponents", RegistryValueKind.String);
                if (!reg1.Success) return reg1;
                Report("STEP 2/5 desactivar IA en Notepad");
                var reg2 = SetRegistryValue(RegistryHive.LocalMachine, @"SOFTWARE\Policies\WindowsNotepad", "DisableAIFeatures", 1, RegistryValueKind.DWord);
                if (!reg2.Success) return reg2;

 // 2) Desinstalación por pasos con reporte: un solo comando gigante deja
 // la consola muda minutos (Get-AppxPackage -AllUsers + winget son lentos)
 // y si algo falla devuelve "Exit code 1:" sin decir qué paso fue.
                Report("STEP 3/5 desinstalar paquetes Copilot y OfficeHub");
                var copilot = await RunCommandAsync("powershell",
                    "-Command \"$ErrorActionPreference = 'SilentlyContinue'; Get-AppxPackage -AllUsers '*Copilot*' | Remove-AppxPackage -AllUsers; Get-AppxPackage -AllUsers Microsoft.MicrosoftOfficeHub | Remove-AppxPackage -AllUsers; Write-Output 'Copilot removido'\"");
                if (!copilot.Success) return copilot;

 // winget tarda y mete ruido en stderr aunque desinstale bien: sus errores
 // no son fatales (el check final decide por el paquete CoreAI real).
                Report("STEP 4/5 desinstalar Copilot via winget");
                await RunCommandAsync("winget",
                    "uninstall -e --id Microsoft.Copilot --silent --accept-source-agreements --accept-package-agreements");

 // Paso 5/5: CoreAI + servicio + Recall. Los errores por servicio/
 // feature ausente NO son fatales: se toleran con -ErrorAction y el
 // resultado lo decide el check final (¿sigue CoreAI instalado?).
                Report("STEP 5/5 desinstalar CoreAI y desactivar Recall");
                var coreAi = await RunCommandAsync("powershell",
                    "-Command \"$ErrorActionPreference = 'SilentlyContinue'; $Sid = [System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value; $Appx = (Get-AppxPackage -AllUsers MicrosoftWindows.Client.CoreAI).PackageFullName; if ($Appx) { New-Item \\\"HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Appx\\AppxAllUserStore\\EndOfLife\\$Sid\\$Appx\\\" -Force | Out-Null }; if ($Appx) { Get-AppxPackage -AllUsers MicrosoftWindows.Client.CoreAI | Remove-AppxPackage -AllUsers }; Set-Service -Name WSAIFabricSvc -StartupType Disabled -ErrorAction SilentlyContinue; Disable-WindowsOptionalFeature -FeatureName Recall -Online -NoRestart -ErrorAction SilentlyContinue | Out-Null; $left = Get-AppxPackage -AllUsers MicrosoftWindows.Client.CoreAI; if ($left) { Write-Output ('CoreAI sigue instalado: ' + $left.PackageFullName); exit 1 }; Write-Output 'Windows AI Disabled'\"");
