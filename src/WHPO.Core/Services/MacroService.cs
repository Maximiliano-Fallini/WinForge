...
public sealed class MacroService : IMacroService
{
    ...
    {
        // Apagar y re-prender el switch en rápido: el hilo previo puede seguir
        // muriendo (duerme hasta 500 ms antes de ver el flag). Se espera su salida
        // (tope ~600 ms) para no arrancar un segundo loop sobre el mismo estado ni
        // dejar el vigilante muerto por el early-return.
        var previous = _watcherThread;
        if (previous != null && previous.IsAlive)
        {
            try { previous.Join(600); } catch { }
            if (previous.IsAlive)
            {
                _logging.LogWarning("MacroService: el vigilante previo no terminó; el arranque queda pendiente.");
                return;
            }
        }
    ...
    public void StopHotkeyWatcher()
    {
        ...
        _isEnabled = null;
        // La referencia al hilo SE CONSERVA (no se anula): StartHotkeyWatcher la
        // usa para esperar su salida antes de arrancar el loop nuevo (apagar y
        // re-prender el switch en rápido no debe dejar dos loops ni ninguno).
    }
