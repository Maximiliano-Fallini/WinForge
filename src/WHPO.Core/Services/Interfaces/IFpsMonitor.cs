...
public interface IFpsMonitor
{
    ...
    /// milisegundos sobre el buffer de frametimes (métrica estándar de "FPS
    /// mínimos"). A diferencia de GetFps (mediana, que disimula los hitches), un
    /// lagazo SÍ hunde el promedio de su ventana. La barra horizontal la usa con
    /// 3000 ms; devuelve 0 si no hay datos suficientes.
    /// </summary>
    double GetWorstFpsWindow(int pid, double windowMs);
