...
public class NavigationService : INavigationService
{
    ...
    {
        NavigateToInternal(pageKey, null, hasParameter: false);
    }

    public void NavigateTo(string pageKey, object? parameter)
        => NavigateToInternal(pageKey, parameter, hasParameter: parameter != null);

    private void NavigateToInternal(string pageKey, object? parameter, bool hasParameter)
    {
        if (_frame == null)
        {
        ...
        try
        {
            // WinUI 3 diferencia Frame.Navigate(tipo) de Frame.Navigate(tipo,
            // null): la segunda sobrecarga puede terminar en el fail-fast nativo
            // "unexpected parameters". Solo pasamos un parámetro si es real.
            if (hasParameter)
