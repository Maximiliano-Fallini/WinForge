...
public sealed class FpsMonitor : IFpsMonitor, IDisposable
{
    ...
    // Ventana de suavizado del FPS: 3 SEGUNDOS en tiempo (no N frames). A 60 fps
    // son ~180 frames, a 144 ~430 — la ventana SIEMPRE cubre los últimos 3 s,
    // lo que da un contador estable (más "latencia") frente a hitches cortos.
    private const long FpsSmoothingTicks = 30_000_000; // 3 segundos en unidades de 100 ns
    ...
    private void TryRecordPresent(TraceEvent e, int pid, DateTime timestamp)
    {
        ...
        // Descartar deltas espurios: < 0.1 ms (duplicados) o > 4 s (alt-tab / pausa
        // larga fuera del juego). Hasta 4 s se cuentan: un "lagazo" por congelado
        // de 2-3 s debe reflejarse en el FPS y en el ↓mín.
        if (dtMs < 0.1 || dtMs > 4000) return;
    ...
    public double GetFps(int pid)
    {
        ...
        lock (stats.Frames)
        {
            int n = stats.Frames.Count;
            if (n == 0) return 0;
            // Mediana de los frametimes de la ventana de 3 SEGUNDOS (en tiempo ETW,
            // no N frames): a 60 fps son ~180 frames y a 144 ~430 — la ventana
            // SIEMPRE cubre los últimos 3 s reales de presentación.
            long newestTicks = stats.Frames.EtwTickAt(n - 1);
            int start = n - 1;
            while (start > 0 && newestTicks - stats.Frames.EtwTickAt(start - 1) <= FpsSmoothingTicks)
                start--;
            int count = n - start;
            var recent = new double[count];
            for (int i = 0; i < count; i++) recent[i] = stats.Frames.MsAt(start + i);
            Array.Sort(recent);
    ...
    /// <summary>
    /// Peor FPS promediado en una ventana deslizante de ~<paramref name="windowMs"/>
    /// sobre el buffer de frametimes. Es la métrica estándar de "FPS mínimos" con
    /// la sensibilidad que pida la ventana: GetFps usa la MEDIANA de los últimos
    /// 3 s — un hitch aislado no mueve la mediana, pero SÍ hunde el promedio de
    /// su ventana. La barra horizontal la usa con 3000 ms (↓ = peor promedio de
    /// ~3 s del buffer reciente). Se recalcula en cada muestreo sobre el BUFFER
    /// actual (no acumula sesión), así el valor cambia con las condiciones y no
    /// queda pegado en el peor de toda la sesión. O(1 pasada) sobre ≤900 muestras.
    /// </summary>
    public double GetWorstFpsWindow(int pid, double windowMs)
    {
        if (!_running || pid <= 0 || !_processes.TryGetValue(pid, out var stats)) return 0;
        lock (stats.Frames)
        {
            int n = stats.Frames.Count;
            if (n < 10) return 0; // menos de ~10 frames: nada representativo

            // Frames cronológicos (los dtMs son deltas ETW positivos y crecientes).
            // La ventana [left..right] cubre el tiempo sumMs = etw(right)-etw(left-1)
            // (suma de los dtMs del rango). FPS de la ventana = count / (sumMs/1000).
            double worst = double.MaxValue;
            double windowAcc = 0;
            int left = 0;
            for (int right = 0; right < n; right++)
            {
                windowAcc += stats.Frames.MsAt(right);
                // Acotar la ventana a ~windowMs recortando desde la izquierda.
                while (left < right && windowAcc - stats.Frames.MsAt(left) > windowMs)
                {
                    windowAcc -= stats.Frames.MsAt(left);
                    left++;
                }
                int count = right - left + 1;
                // Mínimo de frames por ventana para no puntuar ráfagas triviales.
                if (count >= 5 && windowAcc > 0)
                {
                    double fpsInWindow = count * 1000.0 / windowAcc;
                    if (fpsInWindow < worst) worst = fpsInWindow;
                }
            }
            return worst < double.MaxValue ? worst : 0;
        }
    }
