...
public sealed class OverlayMetricsService : IOverlayMetricsService, IDisposable
{
    ...
    // FPS máximo de la sesión del juego actual (se reinicia al cambiar de juego).
    // El mínimo NO se acumula en sesión: se recalcula en CADA muestreo como el
    // peor promedio de una ventana de ~3 s del buffer reciente (ver Sample), así
    // el ↓ de la barra cambia con las condiciones y no queda pegado en el peor
    // valor de toda la sesión.
    private int _fpsSessionPid;
    private double _fpsSessionMax;
    private const double FpsMinWindowMs = 3000.0;

    // VRAM: total dedicado (DXGI, resuelta una vez en EnsureHardware) y usada
    // (contadores "GPU Adapter Memory" → "Dedicated Usage", en MB).
    private readonly List<PerformanceCounter> _gpuMemCounters = new();
    private double _gpuVramTotalMb;
    ...
    private void EnsureCounters()
    {
        ...
        // VRAM usada: contadores "GPU Adapter Memory" → "Dedicated Usage" (la misma
        // fuente que el Administrador de tareas). Hay una instancia por proceso:
        // la suma de todas es el uso dedicado total del adaptador.
        if (_gpuMemCounters.Count == 0)
        {
            ...
            {
                _logging.LogWarning($"OverlayMetrics: VRAM usada no disponible: {ex.Message}");
            }
        }
    }
    ...
    private void Sample()
    {
        try
        {
            ...
            try
            {
                ...
                ramTotalMb = mem.TotalPhysicalMB;
            }
            catch { }

            // VRAM usada (contadores "Dedicated Usage", reportan bytes).
            double gpuMemUsedMb = 0;
            foreach (var counter in _gpuMemCounters)
            {
                try { gpuMemUsedMb += counter.NextValue(); } catch { }
            }
            gpuMemUsedMb /= 1024.0 * 1024.0;
            ...
            // FPS máximo DE LA SESIÓN del juego actual (se reinicia al cambiar de juego).
            // El mínimo NO se acumula: se calcula EN el snapshot como el peor
            // promedio de una ventana de ~3 s del buffer (GetWorstFpsWindow):
            // refleja el estado actual (y los lagazos) y no queda fijo en el peor
            // valor de toda la sesión.
            if (gamePid > 0 && fps > 0)
            {
                if (_fpsSessionPid != gamePid)
                {
                    _fpsSessionPid = gamePid;
                    _fpsSessionMax = 0;
                }
                if (fps > _fpsSessionMax) _fpsSessionMax = fps;
            }
            else
            {
                _fpsSessionPid = 0;
            ...
            _latest = new OverlayMetrics(
                ...
                FpsMax: _fpsSessionMax,
                // ↓Mín RECALCULADO en cada muestreo: peor promedio de ~3 s del
                // buffer (GetWorstFpsWindow) — cambia con las condiciones actuales
                // y captura los lagazos; 0 si todavía no hay datos.
                FpsMin: gamePid > 0 && fps > 0
                    ? _fpsMonitor.GetWorstFpsWindow(gamePid, FpsMinWindowMs)
                    : 0,
                GpuMemUsedMb: gpuMemUsedMb,
                GpuVramTotalMb: _gpuVramTotalMb,
    ...
    private void EnsureHardware()
    {
        ...
        try
        {
            ...
            _gpuVramTotalMb = primary != null ? primary.DedicatedMemoryBytes / (1024.0 * 1024.0) : 0;
