...
///  1. El catálogo vive en el repo (ProjectEndpoints.ComponentCatalog, servido por
...
{
    // La URL del repositorio la define ProjectEndpoints (único lugar a tocar si el
    // proyecto se muda de repo).
    ...
    public string CatalogUrl =>
        _settings.Get("workshop.catalogUrl", ProjectEndpoints.ComponentCatalog);
    ...
    /// </summary>
    private static readonly HashSet<string> RetiredComponentIds = new(StringComparer.OrdinalIgnoreCase)
    {
        "latencia"
    };
        ...
        catch (Exception ex)
        {
            ...
            try
            {
                if (File.Exists(CachePath))
                {
                    ...
                    if (cached != null)
                    {
                        // La copia local también se filtra: si el usuario está offline, el
                        // componente retirado no puede reaparecer por estar en el cache.
                        DropRetiredComponents(cached);
    ...
    /// <summary>
    /// Saca del catálogo los componentes retirados y desinstala los que estuvieran instalados en
    /// ESTA máquina (el registro vive en modules.json, así que la limpieza es idempotente: la
    /// segunda vez no encuentra nada). La carpeta se renombra a *.uninstalled-* y el arranque
    /// siguiente la borra, igual que una desinstalación normal.
    /// </summary>
    private void DropRetiredComponents(ComponentCatalog catalog)
    {
        try
        {
            catalog.Components.RemoveAll(entry => RetiredComponentIds.Contains(entry.Id ?? ""));
            foreach (var id in RetiredComponentIds)
            {
                if (GetInstalled().Any(r => string.Equals(r.Id, id, StringComparison.OrdinalIgnoreCase)))
                {
                    _logging.LogInfo($"Workshop: '{id}' está retirado: se desinstala solo.");
                    Uninstall(id);
                }
            }
        }
        ...
        {
            _logging.LogWarning($"Workshop: no se pudieron filtrar los componentes retirados: {ex.Message}");
        }
    }
