...
public sealed partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
...
#if DEBUG
        // Traza del detector del halo, solo en builds de desarrollo: al cruzar de elemento deja
        // en app.log el host elegido o el motivo del rechazo, así un halo que no aparece se
        // diagnostica leyendo el log en vez de inspeccionar el árbol visual a mano.
        RevealEffect.Trace = msg => _loggingService.LogInfo(msg);
#endif
        ...
        if (_navigationService is NavigationService ns)
        {
            ...
            ns.RegisterPage("tcp", typeof(TcpPage));
            ...
            ns.RegisterPage("ventiladores", typeof(FanControlPage));
        ...
        DispatcherQueue.TryEnqueue(() =>
        {
            ...
            // (resolución del temporizador, limpieza automática de memoria y curvas
            // de ventilador aplicadas).
            _ = ApplyAutoStartFeaturesAsync();

            // Vigilante del hilo de UI: si la interfaz deja de responder, queda en el log.
            StartUiWatchdog();
    ...
    private void SetupTrayIcon()
    {
        try
        {
            ...
            // Tooltip estático: con métricas apagadas el ícono no sondea hardware.
            UpdateTrayStatus();
    ...
    private void BuildTrayMenu(ContextMenuStrip menu)
    {
        ...
        // Cinturón y tirantes: el servicio ya limpia las entradas vacías, pero filtrar
        // acá también para que ninguna entrada sin exe llegue al menú (sería un ítem
        // sin nombre ni ícono).
        ...
        else
        {
            foreach (var exe in favorites)
            {
                string label = _favNames.TryGetValue(exe, out var n) && !string.IsNullOrEmpty(n)
                    ? n
                    : string.IsNullOrWhiteSpace(Path.GetFileNameWithoutExtension(exe))
                        ? I18n.T("Juego desconocido")
    ...
    private void HideWindow()
    {
        ...
        // Pausa real de los timers DE PÁGINA (IBackgroundPausable): Stop de verdad
        // en vez de dejarlos tickeando en segundo plano. Los features de proceso
        // (curvas, AutoCleanup, timer resolution, WLAN keep-alive, overlay,
        // GameBoost) NO se tocan: se apagan con su propio switch.
        if (ContentFrame.Content is IBackgroundPausable pausable)
        {
            try { pausable.PauseBackgroundTimers(); }
            catch (Exception ex) { _loggingService.LogWarning($"MainWindow: pausar timers de página: {ex.Message}"); }
        }
    ...
    private void ShowWindow()
    {
        ...
        // Reanudar los timers DE PÁGINA pausados al ocultar (cada página decide si
        // le corresponde: datos cargados, sin pausa manual del usuario, etc.).
        if (ContentFrame.Content is IBackgroundPausable pausable)
        {
    ...
    /// <summary>
    /// Estado del icono de bandeja. Con métricas apagadas ("tray.optimizePerformance"
    /// true, el default): sin métricas y huella mínima (tooltip estático + liberar
    /// memoria al ocultar la ventana). Encendidas: muestra uso de CPU, memoria y
    /// temperatura al pasar el cursor, con sondeos periódicos (más CPU en fondo).
    /// </summary>
    internal void UpdateTrayStatus()
    {
        if (_notifyIcon == null) return;

        // Mismo default que ConfiguracionPage (true): si la clave no existe en el
        // settings, ambos lados asumen el estado "prendido" del toggle. Con defaults
        // distintos el toggle se veía prendido pero las métricas arrancaban igual.
        var optimize = _settingsService.Get("tray.optimizePerformance", true);

        if (optimize)
        {
            StopTrayMetrics();
            _notifyIcon.Text = IsWindowVisible ? "WinForge" : $"WinForge — {I18n.T("en segundo plano")}";
            if (!IsWindowVisible)
                TrimProcessMemory();
        }
        else
    ...
    /// Solo se invoca al ocultar la ventana con las métricas de bandeja apagadas.
    /// </summary>
 ...
 // que la X: ocultar a la bandeja en vez de minimizar a la barra.
    ...
    // Reanudar de una suspensión (el chip puede haber vuelto al modo BIOS).
    private const uint WM_POWERBROADCAST = 0x0218;
    private const long PBT_APMRESUMESUSPEND = 0x0007;
    private const long PBT_APMRESUMECRITICAL = 0x0006;
 ...
 /// El modo "sin métricas" aplica su limpieza al ocultar.
 /// </summary>
 ...
 // error es 0) el reemplazo funcionó.
    ...
    /// <summary>
    /// Revalida el control de ventiladores tras reanudar de una suspensión. Corre en
    /// segundo plano: el mensaje de Windows no debe esperar a la reapertura del chip.
    /// </summary>
    private async Task RevalidateFanControlAsync()
    {
        try
        {
            var fanService = App.Services.GetRequiredService<IFanControlService>();
            await Task.Run(() => fanService.ResumeFromSleep());
        }
        catch (Exception ex)
        {
            _loggingService.LogWarning($"Revalidación del control de ventiladores tras la suspensión falló: {ex.Message}");
        }
    }

    // ===== Vigilante del hilo de UI =====
    // Un cuelgue no deja ninguna pista: el hilo que se cuelga es justamente el que
    // escribe los logs y el que cierra la app. Este latido va en el hilo de UI y un
    // vigilante lo revisa desde un hilo de fondo; si el latido se corta, se registra
    // cuánto llevaba sin responder — así un cuelgue queda identificado en app.log.
    private const int UiStallThresholdMs = 5000;
    private const int UiStallReportCooldownMs = 10000;
    private long _uiHeartbeatMs;
    private long _lastUiStallReportMs;
    private DispatcherQueueTimer? _uiHeartbeatTimer;
    private System.Threading.Timer? _uiWatchdogTimer;

    private void StartUiWatchdog()
    {
        _uiHeartbeatMs = Environment.TickCount64;

        _uiHeartbeatTimer = DispatcherQueue.CreateTimer();
        _uiHeartbeatTimer.Interval = TimeSpan.FromMilliseconds(500);
        _uiHeartbeatTimer.Tick += (s, e) =>
            System.Threading.Interlocked.Exchange(ref _uiHeartbeatMs, Environment.TickCount64);
        ...
        _uiWatchdogTimer = new System.Threading.Timer(_ =>
        {
            try
            {
                ...
                long idle = now - System.Threading.Interlocked.Read(ref _uiHeartbeatMs);
                if (idle < UiStallThresholdMs) return;
                if (now - System.Threading.Interlocked.Read(ref _lastUiStallReportMs) < UiStallReportCooldownMs) return;

                System.Threading.Interlocked.Exchange(ref _lastUiStallReportMs, now);
                _loggingService.LogWarning($"Interfaz sin responder desde hace {idle / 1000} s (posible cuelgue)");
            }
            catch { }
    ...
    private IntPtr MinimizeSubclassProc(IntPtr hWnd, uint uMsg, IntPtr wParam, IntPtr lParam, IntPtr uIdSubclass, IntPtr dwRefData)
    {
        try
        {
            ...
            // Volver de dormir: el firmware puede haber devuelto los canales al modo
            // BIOS y el acceso al chip puede quedar viejo, así que el control de
            // ventiladores se revalida y las curvas se reaplican al instante.
            if (uMsg == WM_POWERBROADCAST &&
                ((long)wParam == PBT_APMRESUMEAUTOMATIC || (long)wParam == PBT_APMRESUMESUSPEND || (long)wParam == PBT_APMRESUMECRITICAL))
            {
                _ = RevalidateFanControlAsync();
            }
        }
        catch { }
    ...
    private async Task UpdateTrayTooltipAsync()
    {
        ...
        // Blindaje: si las métricas se apagaron con el timer ya en marcha,
        // no escribir métricas (pisa el tooltip estático del modo optimizado).
        if (_settingsService.Get("tray.optimizePerformance", true))
        {
            UpdateTrayStatus();
            return;
        }
 ...
 // La resolución efectiva se MIDE (~30-60 ms): fuera del hilo de UI.
    ...
    /// <summary>
    /// Reaplica al arrancar lo que el usuario dejó encendido en la sesión anterior:
    /// la resolución del temporizador, la limpieza automática de memoria y las curvas
    /// de ventilador que estaban aplicadas. Las tres viven en este proceso: al
    /// cerrarlo, Windows revierte la resolución, la limpieza se detiene y los canales
    /// de ventilador vuelven al BIOS.
    /// </summary>
    private async Task ApplyAutoStartFeaturesAsync()
    {
        try
        {
            ...
            // Curvas de ventilador que quedaron aplicadas. Puerta de entrada barata
            // (solo settings): si no hay ninguna, no se abre el hardware (LHM/bus
            // ISA) en absoluto. En segundo plano: abrir el chip y escribir el duty
            // puede tardar, y el método reintenta un rato si el hardware todavía
            // no levantó al arrancar.
            var fanService = App.Services.GetRequiredService<IFanControlService>();
            if (!fanService.HasCurvesToReapply())
            {
                _loggingService.LogDebug("Curvas de ventilador: sin curvas guardadas, no se abre el hardware al iniciar");
            }
            else
            {
                int reapplied = await Task.Run(() => fanService.ReapplyCurvesOnStartup());
                if (reapplied > 0)
                    _loggingService.LogInfo($"Curvas de ventilador reaplicadas al iniciar: {reapplied}");
            }
        }
        catch (Exception ex)
        {
    ...
    private NavigationViewItem CreateNavItem(WHPO.Core.Components.IWinForgeComponent comp) => new()
    {
        Content = comp.Name,
        Tag = comp.Id,
        Icon = string.Equals(comp.Id, "ventiladores", StringComparison.OrdinalIgnoreCase)
            ? FanIcon.CreateIcon(16)
            : new FontIcon
            {
    ...
    private async void LanguageButton_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            await ShowLanguageMenuAsync();
        }
        catch (Exception ex)
        {
            _loggingService.LogWarning($"Idiomas: no se pudo abrir el menú de idiomas: {ex.Message}");
        }
    }

    /// <summary>
    /// Menú de idiomas del navbar: los disponibles (embebidos + packs instalados) para
    /// elegir, los descargables del catálogo (se bajan y se activan al elegirlos) y la
    /// opción de quitar los packs instalados.
    ///
    /// Se arma en cada apertura: los descargables salen del catálogo (la primera vez va
    /// a la red y después queda en memoria; si no hay red, usa la copia local). Antes
    /// esto solo recorría I18n.Languages —los embebidos y los packs YA instalados—, así
    /// que los idiomas descargables no existían como opción en ningún lado.
    /// </summary>
    private async Task ShowLanguageMenuAsync()
    {
        ...
        // TODOS los idiomas con la misma fila: bandera + nombre, y a la derecha la acción que
        // corresponda (✓ activo · X quitar · descarga con spinner). Los descargables ya no son
        // ítems distintos: antes perdían la bandera, llevaban el peso en el texto y el "botón"
        // de descarga era el ícono mismo (no había forma de distinguir acción de identidad).
        var rows = new StackPanel { Spacing = 4, Width = 260 };

        foreach (var code in I18n.Languages)
            rows.Children.Add(BuildLanguageRow(code, entry: null));

        var downloadable = LanguagePacks.Downloadable();
        if (downloadable.Count > 0 && I18n.Languages.Any())
            rows.Children.Add(new Border
            {
                Height = 1,
                Margin = new Thickness(2, 3, 2, 3),
                Background = ThemeBrushes.Get("SensorGridLineBrush")
            });
        foreach (var entry in downloadable)
            rows.Children.Add(BuildLanguageRow(entry.Code, entry));
    ...
    private FrameworkElement BuildLanguageRow(string code, LanguageCatalogEntry? entry)
    {
        ...
        bool builtin = I18n.BuiltinLanguages.Contains(code, StringComparer.OrdinalIgnoreCase);
        bool installed = !builtin && LanguagePacks.IsInstalled(code);
        ...
        var flag = Flags.GetImage(code);
        if (flag is not null) grid.Children.Add(flag);

        var name = new TextBlock
        {
            Text = LanguagePacks.DisplayName(code),
            FontSize = 13,
            FontWeight = active ? WinFontWeights.SemiBold : WinFontWeights.Normal,
            VerticalAlignment = VerticalAlignment.Center,
            TextTrimming = TextTrimming.CharacterEllipsis
        };
        var nameButton = new WinButton
        {
            Content = name,
        ...
        // El nombre activa el idioma solo si ya está disponible: los descargables usan el
        // botón de descarga de la derecha (el nombre queda igual de visible que en el resto).
        if (entry is null)
        ...
        {
            // Descarga: el botón deja lugar al spinner mientras baja (misma celda, mismo
            // tamaño, así la fila no salta). Al terminar, el menú se rearma.
            ...
            var download = new WinButton
            {
                Background = new WinBrush(Microsoft.UI.Colors.Transparent),
                BorderThickness = new Thickness(0),
                Padding = new Thickness(6),
                VerticalAlignment = VerticalAlignment.Center,
                Content = new FontIcon { Glyph = "\uE896", FontSize = 13, Foreground = ThemeBrushes.Get("AccentBrush") }
            };
            ToolTipService.SetToolTip(download, I18n.T("Instalar"));
            var action = new StackPanel { Orientation = WinOrientation.Horizontal, Spacing = 2 };
            action.Children.Add(spinner);
            action.Children.Add(download);
            var target = entry;
            download.Click += async (_, _) => await DownloadLanguageAsync(target, spinner, download, name);
    ...
    /// <summary>
    /// Descarga un pack de idioma desde su fila: mientras baja, el botón de descarga deja
    /// lugar al spinner; al terminar, el idioma queda activo y el menú se rearma (la fila pasa
    /// a "instalado", con su X para quitarlo).
    /// </summary>
    private async Task DownloadLanguageAsync(LanguageCatalogEntry entry, ProgressRing spinner, WinButton download, TextBlock name)
    {
        ...
        try
        {
            ...
            if (outcome.Success)
            {
                I18n.SetLanguage(entry.Code, _settingsService);
                ApplyLanguageButton();
                _loggingService.LogInfo($"Idiomas: pack {entry.Code} v{entry.Version} instalado y activado.");
                await ShowLanguageMenuAsync();
                return;
            }
            name.Text = outcome.Error ?? LanguagePacks.DisplayName(entry.Code);
            _loggingService.LogWarning($"Idiomas: descarga de {entry.Code} fallida: {name.Text}");
        }
        catch (Exception ex)
        {
            name.Text = ex.Message;
            _loggingService.LogWarning($"Idiomas: descarga de {entry.Code}: {ex.Message}");
        }
        finally
        {
