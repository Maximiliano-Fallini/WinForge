...
public partial class App : Application
{
    ...
    public App()
    {
        ...
        var gameBoost = Services.GetService<IGameBoostService>();

        // Si quedó un journal de una partida que no se cerró bien (crash o kill del
        // proceso con el boost activo), devolver el estado previo acá: los servicios y
        // las tareas no deben quedar colgados hasta reiniciar Windows.
        if (gameBoost != null)
            _ = gameBoost.RecoverPendingJournalAsync();
    ...
    protected override void OnLaunched(LaunchActivatedEventArgs args)
    {
        ...
        // Vigilante de atajos de macros: LAZY (no se arranca acá). Lo pide la página
        ...
        void CreateMainWindow()
        {
            ...
            // Se recupera en segundo plano y se activa al terminar: sin esto el usuario
            // vería la app en inglés sin entender por qué. Si no hay red, no pasa nada
            // y el menú de idioma queda con la descarga atenuada.
            _ = WHPO_UI.LanguagePacks.RestoreSavedLanguageAsync(
                settingsService,
                Services.GetRequiredService<ILoggingService>());
    ...
    {
        // Marca en el log el arranque del apagado: sin esto un cierre no deja rastro
        // y es indistinguible de una muerte del proceso (útil al depurar cuelgues).
        try { Services.GetService<ILoggingService>()?.LogInfo("Ventana principal cerrada: apagando la app"); } catch { }
        ...
        try { Services.GetService<ILoggingService>()?.LogInfo("Apagado terminado"); } catch { }
    }
}
