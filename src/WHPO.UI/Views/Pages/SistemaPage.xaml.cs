...
public sealed partial class SistemaPage : Page, IBackgroundPausable
{
    ...
    // Estado de seguridad del firmware (TPM / Secure Boot / IOMMU / VBS) para poder
    // re-aplicarlo al cambiar de idioma. -1 en VbsStatus = dato desconocido (WMI no
    // disponible), para no mostrar "Desactivado" sin datos.
    private SecurityFeatures _securityFeatures = new(false, false, "", false, false, false, -1, false, false);
    ...
    public SistemaPage()
    {
        ...
        catch (Exception ex)
        {
            ...
            // Sin logging service no hay registro: dejar constancia directa en
            // errors.log para que un fallo del constructor no sea invisible.
            try
            {
                var errLog = System.IO.Path.Combine(
                    Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "WHPO", "errors.log");
                System.IO.File.AppendAllText(errLog, $"[{DateTime.Now:O}] SistemaPage ctor: {ex}{Environment.NewLine}");
            }
            catch { }
        }
    }
 ...
 // lógica de primaria del servicio decide, como siempre).
        ...
        await RevealCardAsync(VbsSkeleton, VbsContent);
    ...
    private FrameworkElement?[] AllSkeletons => new FrameworkElement?[]
    {
        ...
        TpmSkeleton, SecureBootSkeleton, IommuSkeleton, VbsSkeleton
    };
    ...
    private FrameworkElement?[] AllContents => new FrameworkElement?[]
    {
        ...
        TpmContent, SecureBootContent, IommuContent, VbsContent
    };
    ...
    private void ApplySecurityFeatures()
    {
        try
        {
            ...
            // VBS / HVCI: 0 desactivado, 1 configurado sin correr, 2 en ejecución,
            // -1 sin dato (WMI no disponible: NO se afirma que esté desactivado).
            switch (_securityFeatures.VbsStatus)
            {
                case 2:
                    VbsStatusText.Text = I18n.T("Activo");
                    VbsIndicator.Fill = StatusBrush("SuccessBrush");
                    break;
                case 1:
                    VbsStatusText.Text = I18n.T("Configurado");
                    VbsIndicator.Fill = StatusBrush("WarningBrush");
                    break;
                case 0:
                    VbsStatusText.Text = I18n.T("Desactivado");
                    VbsIndicator.Fill = StatusBrush("ErrorBrush");
                    break;
                default:
                    VbsStatusText.Text = I18n.T("No detectado");
                    VbsIndicator.Fill = StatusBrush("WarningBrush");
                    break;
            }
            // Detalle: HVCI (integridad de memoria) corre dentro de VBS. Se informa
    ...
    /// <summary>
    /// Pausa de bandeja (IBackgroundPausable): Stop real del monitoreo — el tick
    /// de 1 s (WMI/LHM) no debe seguir con la ventana oculta.
    /// </summary>
    public void PauseBackgroundTimers() => StopMonitoring();

    /// <summary>
    /// Reanudación desde bandeja (IBackgroundPausable): solo si la página está
    /// activa (es la visible) y la ventana volvió a mostrarse.
    /// </summary>
    public void ResumeBackgroundTimers()
    {
        if (_isActive && _dataLoaded && App.MainWindowInstance?.IsWindowVisible == true)
            StartMonitoring();
    }
