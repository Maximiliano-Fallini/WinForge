...
/// </summary>
public sealed partial class ProcesosPage : Page, IBackgroundPausable
{
    ...
    // ===================== Skeleton de carga =====================
    // Filas fantasma que ocupan el hueco de la tabla mientras llega el PRIMER snapshot. Se
    // retiran con la primera lista real, con un mínimo visible para que no parpadee.
    private bool _skeletonActive;
    private long _skeletonShownAtMs;
    private readonly List<Storyboard> _skeletonPulse = new();
    private const int MinSkeletonVisibleMs = 450;
    private const int SkeletonRows = 12;
    ...
    protected override void OnNavigatedTo(Microsoft.UI.Xaml.Navigation.NavigationEventArgs e)
    {
        ...
        UpdateFilterButtons();
        UpdateTreeButton();
        // Primera carga (o vuelta a la página después de liberar las filas): la tabla se cubre
        // con el skeleton hasta que llegue el primer snapshot.
        if (_rows.Count == 0) ShowRowsSkeleton();
        _ = RefreshAsync();
    }
    ...
    protected override void OnNavigatedFrom(Microsoft.UI.Xaml.Navigation.NavigationEventArgs e)
    {
        ...
        ClearRows();
        // La animación del skeleton no debe seguir corriendo fuera de pantalla: al volver,
        // OnNavigatedTo lo retoma desde cero.
        _skeletonActive = false;
        RowsSkeleton.Visibility = Visibility.Collapsed;
        StopRowsSkeletonPulse();
    ...
    /// <summary>
    /// Muestra las filas fantasma: la PRIMERA lectura de la tabla (snapshot nativo + contadores
    /// de GPU + reglas del disco) puede tardar y la tabla vacía parecía una página rota.
    /// Las filas se construyen una sola vez y se reutilizan.
    /// </summary>
    private void ShowRowsSkeleton()
    {
        if (_skeletonActive || _rows.Count > 0) return;
        try
        {
            if (RowsSkeleton.Children.Count == 0) BuildRowsSkeleton();
            _skeletonShownAtMs = Environment.TickCount64;
            RowsSkeleton.Visibility = Visibility.Visible;
            _skeletonActive = true;
            StartRowsSkeletonPulse();

            // Seguridad: si el primer snapshot no llegara (WMI lento, contadores de GPU
            // colgados), el skeleton se retira igual a los 5 s en vez de dejar la tabla tapada.
            _ = Task.Delay(5000).ContinueWith(_ => DispatcherQueue.TryEnqueue(() =>
            {
                if (_skeletonActive) HideRowsSkeleton(force: true);
            }));
        }
        catch (Exception ex)
        {
            _loggingService.LogWarning($"ProcesosPage: skeleton de carga: {ex.Message}");
            _skeletonActive = false;
            RowsSkeleton.Visibility = Visibility.Collapsed;
        }
    }

    /// <summary>Retira el skeleton. Sin <paramref name="force"/>, respeta un mínimo visible para
    /// que un snapshot que llega en 100 ms no se vea como un parpadeo.</summary>
    private void HideRowsSkeleton(bool force = false)
    {
        if (!_skeletonActive) return;
        long elapsed = Environment.TickCount64 - _skeletonShownAtMs;
        if (!force && elapsed < MinSkeletonVisibleMs)
        {
            int wait = (int)(MinSkeletonVisibleMs - elapsed);
            _ = Task.Delay(wait).ContinueWith(_ => DispatcherQueue.TryEnqueue(() => HideRowsSkeleton(force: true)));
            return;
        }
        _skeletonActive = false;
        RowsSkeleton.Visibility = Visibility.Collapsed;
    ...
    /// <summary>Filas fantasma con los MISMOS anchos de columna que la tabla real, para que la
    /// silueta coincida con lo que va a aparecer. Los bloques no llenan la celda y varían de
    /// largo: todo del mismo tamaño se ve artificial.</summary>
    private void BuildRowsSkeleton()
    {
        RowsSkeleton.Children.Clear();
        var fill = (Brush)Application.Current.Resources["ControlFillColorSecondaryBrush"];

        for (int i = 0; i < SkeletonRows; i++)
        {
            var row = new Grid { Width = TotalWidth, Height = RowHeight, HorizontalAlignment = HorizontalAlignment.Left };
            for (int p = 0; p < Columns.Length; p++)
            {
                ...
                double span = Math.Max(30, w - 40);
                double textW = Math.Min(w - 26, 52 + ((i * 29 + p * 17) % (int)span));
                var block = new Rectangle
                {
                    Width = Math.Max(18, textW),
                    Height = 8,
                    RadiusX = 4,
                    RadiusY = 4,
                    Fill = fill,
                ...
                Grid.SetColumn(block, p);
                row.Children.Add(block);
            }
            RowsSkeleton.Children.Add(row);
        }
    }

    /// <summary>Pulso suave de las filas fantasma (1.0 → 0.35 en 900 ms, ida y vuelta, como el
    /// resto de la app). Se anima cada FILA y no cada bloque: son 12 animaciones en vez de ~150,
    /// y las filas no tienen fondo propio (el fondo lo pone el panel).</summary>
    private void StartRowsSkeletonPulse()
    {
        try
        {
            StopRowsSkeletonPulse();
            foreach (var child in RowsSkeleton.Children)
            {
                ...
                _skeletonPulse.Add(sb);
                sb.Begin();
            }
        }
        catch (Exception ex)
        {
            _loggingService.LogWarning($"ProcesosPage: pulso del skeleton: {ex.Message}");
        }
    }

    private void StopRowsSkeletonPulse()
    {
        foreach (var sb in _skeletonPulse)
        {
            try { sb.Stop(); } catch { }
        }
        _skeletonPulse.Clear();
    }
    ...
    /// <summary>
    /// Pausa de bandeja (IBackgroundPausable): Stop real del refresh de 1 s, pero
    /// SIN destruir el timer ni limpiar filas (el contenido se conserva al volver).
    /// Respeta la pausa manual del usuario (_paused): si pausó, sigue pausado.
    /// </summary>
    public void PauseBackgroundTimers() => _timer?.Stop();

    /// <summary>Reanudación desde bandeja: solo con ventana visible y sin pausa manual.</summary>
    public void ResumeBackgroundTimers()
    {
        if (!_paused && App.MainWindowInstance?.IsWindowVisible == true)
            _timer?.Start();
    }
    ...
    private void UpdateRows(List<ProcInfo> items)
    {
        ...
        // Llegó la lista: el skeleton cumplió su función.
        HideRowsSkeleton();
    }
