...
public sealed partial class OptimizacionesPage : Page
{
    ...
    // sigue viva mientras un tweak tarda (PowerShell puede tardar minutos sin
    // emitir una sola línea). DispatcherTimer vive en Microsoft.UI.Xaml.
    ...
    private void BuildPresets()
    {
        ...
        foreach (var (glyph, name, subtitle, tweaks) in presets)
        {
            ...
            // El halo del efecto se dibuja sobre el HOST que detecta el recorrido del árbol, y el
            // recorrido probado en esta página son los Border: se envuelve el botón en uno del
            // mismo radio (sin fondo propio, para no tapar el del botón) y se marca ese Border.
            // Así el preset recibe el halo sin depender del tipo del control.
            var host = new Border { CornerRadius = new CornerRadius(10), Child = button };
            RevealEffect.Mark(host);
            PresetsPanel.Children.Add(host);
        }
    }
    ...
    private Border BuildTweakCard(TweakInfo tweak)
    {
        ...
        // Halo que sigue al mouse: la card se marca a mano porque queda por debajo del alto
        // mínimo que exige el detector heurístico del efecto (una sola línea de contenido),
        // así que sin el marcador la heurística la descarta y no recibe el halo.
    ...
    private void ApplyThemeToCards()
    {
        ...
        {
            // El preset vive dentro de un Border (que es el host del halo), así que hay que
            // alcanzar el botón tanto si el hijo es el botón como si es su contenedor.
            var button = child as Button ?? (child as Border)?.Child as Button;
            if (button != null)
