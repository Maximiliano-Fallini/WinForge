...
public sealed partial class RedPage : Page, IBackgroundPausable
{
    ...
    /// <summary>
    /// Pausa de bandeja (IBackgroundPausable): el ticker del packet-loss test
    /// (100 ms) no sigue con la ventana oculta. El test en curso se cancela
    /// (sus resultados parciales no tienen UI que los muestre).
    /// NOTA: el keep-alive WLAN (singleton, cada 5 s) NO se pausa: es un feature
    /// explícito que debe seguir con la app minimizada.
    /// </summary>
    public void PauseBackgroundTimers()
    {
        if (_packetLossRunning)
        {
            try { _packetLossCts?.Cancel(); } catch { }
    ...
    /// <summary>
    /// Reanudación desde bandeja: nada que reanudar (el test cancelado no se
    /// reinicia solo; el keep-alive WLAN nunca se pausó). Método vacío a propósito.
    /// </summary>
    ...
    // ===== Helpers compartidos por las cards WLAN (ex-TCP también los usaba; TCP
    // tiene su propia copia desde que es pestaña aparte) =====
    ...
    /// Al lado del título se muestra un botón "?" con tooltip custom (título en negrita
    /// + descripción), colocado abajo del botón.
    /// </summary>
    ...
    // ===== Tooltips WLAN: qué hace cada ajuste y cuándo conviene ON/OFF =====
