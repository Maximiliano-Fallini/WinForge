...
/// <summary>
/// Página "Macros" (componente separado de "Filtro de teclas"): grabación,
/// edición y ejecución de secuencias de teclas y clics con atajo global.
/// El vigilante de atajos (MacroHotkeyService) arranca LAZY al visitar esta
/// página (solo con switch prendido y macros con atajo) y sobrevive a la
/// navegación; apagar el switch mata su hilo.
/// </summary>
public sealed partial class MacrosPage : Page
{
    ...
    protected override void OnNavigatedTo(Microsoft.UI.Xaml.Navigation.NavigationEventArgs e)
    {
        ...
        // Refrescar siempre (pueden cambiar entre visitas). El vigilante de atajos es
        // LAZY: arranca acá solo con switch prendido y macros con atajo (sin eso no
        // hay hilo ni polling en segundo plano).
        try
        {
        ...
        {
            _loggingService.LogWarning($"MacrosPage: no se pudieron cargar las macros: {ex.Message}");
        }

        // Contenido construido en código (lista de macros, editor): re-aplicar al
        // cambiar de idioma. La página usa caché: se suscribe y desuscribe.
    ...
    private void SaveMacroButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            _macroService.UpdateArmedMacros(_macros);
            // Guardar una macro con atajo (con switch prendido) puede ser la primera
            // que justifique el vigilante: intentar arrancarlo (lazy, idempotente).
            _macroHotkeys.EnsureStarted();
        }
        catch (Exception ex)
        {
    ...
    private async void DeleteMacro(MacroDefinition macro)
    {
        ...
        try
        {
            _macroService.Save(_macros);
            _macroService.UpdateArmedMacros(_macros);
            // Si no quedan macros con atajo, no hay nada que vigilar: matar el hilo
            // (el vigilante es lazy y se recrea al guardar/prender de nuevo).
            if (!_macros.Any(m => m.HotkeyVk != 0 && m.Steps.Count > 0))
                _macroHotkeys.Stop();
        }
        catch (Exception ex)
        {
    ...
    private void SetRecordingButtons(bool recording)
    {
        ...
        // Al terminar la grabación, deshabilitar el botón de detener deja al foco sin
        // control: WinUI lo manda al siguiente enfocable y caía en el NumberBox de
        // "delay entre eventos", con el cursor de texto encima del campo. Se devuelve el
        // foco al botón de grabar (nunca en la carga de la página: ahí el botón de
        // detener ya está deshabilitado y no hay foco que rescatar).
        if (!recording && StopRecordButton.IsEnabled)
