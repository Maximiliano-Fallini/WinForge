...
public sealed partial class MemoriaPage : Page, IBackgroundPausable
{
    ...
    /// <summary>
    /// Pausa de bandeja (IBackgroundPausable): Stop real del timer de estadísticas
    /// (2 s) — antes seguía tickeando con un guard barato adentro.
    /// NOTA: la limpieza AUTOMÁTICA (memory.autoStart, timer de proceso) NO se pausa:
    /// es un feature explícito, se detiene con su propio botón.
    /// </summary>
    public void PauseBackgroundTimers() => _memoryStatsTimer?.Stop();

    /// <summary>Reanudación desde bandeja: solo con datos cargados y ventana visible.</summary>
    public void ResumeBackgroundTimers()
    {
        if (_dataLoaded && App.MainWindowInstance?.IsWindowVisible == true)
            _memoryStatsTimer?.Start();
    }
}
