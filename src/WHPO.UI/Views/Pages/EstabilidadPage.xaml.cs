...
/// </summary>
public sealed partial class EstabilidadPage : Page, IBackgroundPausable
{
    ...
    /// <summary>
    /// Pausa de bandeja (IBackgroundPausable): Stop real del muestreo de 1 s.
    /// NOTA: un test de estrés en curso NO se pausa (es un feature explícito del
    /// servicio, no un timer de UI): la cuenta regresiva sigue, solo se congela
    /// el muestreo de los gráficos hasta volver.
    /// </summary>
    public void PauseBackgroundTimers() => StopSampling();

    /// <summary>Reanudación desde bandeja: solo con página cargada y ventana visible.</summary>
    public void ResumeBackgroundTimers()
    {
        if (_loaded && App.MainWindowInstance?.IsWindowVisible == true)
            StartSampling();
    }
