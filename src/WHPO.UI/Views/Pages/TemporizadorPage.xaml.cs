...
public sealed partial class TemporizadorPage : Page, IBackgroundPausable
{
 ...
 // y el flag GlobalTimerResolutionRequest es el que hace que funcione).
    ...
    /// <summary>
    /// Pausa de bandeja (IBackgroundPausable): Stop real del refresh de 500 ms
    /// (cada tick mide ~30-60 ms) — antes seguía tickeando con guard barato.
    /// NOTA: la petición de resolución al kernel (timer.autoStart) NO se pausa:
    /// es un feature explícito, se libera con Detener.
    /// </summary>
    public void PauseBackgroundTimers() => _refreshTimer?.Stop();
