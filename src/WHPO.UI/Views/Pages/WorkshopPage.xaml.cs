...
public sealed partial class WorkshopPage : Page
{
    ...
    private async Task FetchCatalogAsync()
    {
        ...
        // Los textos que trae el propio catálogo (bloque "i18n" de cada componente) se
        // registran ANTES de armar las cards: así el nombre y la descripción salen en el
        // idioma activo sin que el componente tenga que ser una clave de la tabla de la
        // app. Ver reglas_de_desarollo_.
        if (result.Catalog != null)
            Translations.SetComponentStrings(result.Catalog.Components.SelectMany(c => c.Translations()));
        ...
        // El navbar se armó al arrancar, cuando el catálogo todavía no existía: con los
        // textos ya registrados, una re-aplicación lo traduce sin reiniciar la app.
        if (result.Catalog != null) I18n.Reapply();
    }
    ...
    {
        if (card.Instance != null && card.Installed != null) return true;
        if (card.Entry != null) return false; // descargable sin instalar
        if (card.Instance != null)
        {
            var settings = App.Services.GetRequiredService<WHPO.Core.Services.Interfaces.ISettingsService>();
            return !settings.Get("builtin.removed." + card.Id, _registry.RequiresInstall(card.Id));
        }
        return false;
    }
    ...
    private UIElement? BuildCard(CardModel card, string search)
    {
        ...
        // Hover: solo lo no instalado toma el borde de acento; lo instalado
        // conserva su reborde verde permanente como señal de estado.
        root.PointerEntered += (s, e) => { if (!installed) root.BorderBrush = accent; };
        root.PointerExited += (s, e) => { if (!installed) root.BorderBrush = transparentBrush; };
        ...
        header.ColumnDefinitions.Add(new ColumnDefinition { Width = GridLength.Auto });
        // El fan va con el acento, como los demás íconos de las cards del catálogo.
        // El PathIcon del ventilador necesita alineación explícita a la izquierda:
        // sin ella se estira y queda centrado en su columna (los FontIcon sí traen
        // Left por defecto, por eso solo esta card se veía corrida).
        if (string.Equals(card.Id, "ventiladores", StringComparison.OrdinalIgnoreCase))
        {
            var fanIcon = FanIcon.CreatePathIcon(CardIconSize, accent);
            fanIcon.HorizontalAlignment = HorizontalAlignment.Left;
            fanIcon.VerticalAlignment = VerticalAlignment.Top;
