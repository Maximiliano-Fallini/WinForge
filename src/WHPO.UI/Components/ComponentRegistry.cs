...
public sealed class ComponentRegistry
{
    ...
    public bool RequiresInstall(string id)
        => _nonCoreBuiltinIds.Contains(id)
           && !string.Equals(id, "overclockusb", StringComparison.OrdinalIgnoreCase);
    ...
    private static IEnumerable<IWinForgeComponent> CreateBuiltins()
    {
        ...
        // Control de ventiladores: no core (nace sin instalar y se activa desde el
        // Workshop, como el resto de las pestañas de hardware). El navbar y la card
        // del Workshop le ponen el ventilador dibujado a mano en vez del glifo: ver
        // FanIcon y los casos por id en MainWindow.CreateNavItem/WorkshopPage.
        yield return new BuiltinComponent
        {
            Id = "ventiladores", Name = "Ventiladores", IconGlyph = "\uE72E", Category = ComponentCategory.Monitoreo,
            Description = "Curvas PWM por sensor (CPU/GPU/placa) vía driver PawnIO, con autostart al iniciar sesión.",
            PageType = typeof(FanControlPage)
        };
        yield return new BuiltinComponent
        {
